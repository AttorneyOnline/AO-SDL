...
class JsonConfiguration : public ConfigurationBase<Derived> {
  ...
  private:
    ...
    static nlohmann::json any_to_json(const std::any& v) {
        ...
        if (v.type() == typeid(nlohmann::json))       return std::any_cast<nlohmann::json>(v);
    ...
    static std::any json_to_any_typed(const nlohmann::json& j, const std::any& hint) {
        if (hint.type() == typeid(bool)        && j.is_boolean())          return j.get<bool>();
        if (hint.type() == typeid(int)         && j.is_number())           return j.get<int>();
        if (hint.type() == typeid(int64_t)     && j.is_number())           return j.get<int64_t>();
        if (hint.type() == typeid(uint64_t)    && j.is_number())           return j.get<uint64_t>();
        if (hint.type() == typeid(double)      && j.is_number())           return j.get<double>();
        if (hint.type() == typeid(std::string) && j.is_string())           return j.get<std::string>();
        if (hint.type() == typeid(nlohmann::json))                         return j;
