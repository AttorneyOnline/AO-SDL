...
class ShaderAsset : public Asset {
  public:
    ShaderAsset(std::string path, std::string format,
    ...
    void set_uniform_provider(std::shared_ptr<ShaderUniformProvider> p) { uniform_provider_ = std::move(p); }
    const std::shared_ptr<ShaderUniformProvider>& uniform_provider() const { return uniform_provider_; }

    size_t memory_size() const override { return vertex_source_.size() + fragment_source_.size(); }

    bool is_default() const { return vertex_source_.empty() && fragment_source_.empty(); }
